...
parser.add_argument('-a', action="store", dest="action", default='+', choices=['+', '-'])
parser.add_argument('-i', action="store", dest="img_addr", default=MASTER, type=ip_type)
...
with open('nodelist.cfg', "w+") as cfg_file:
    if arg_results.vm_num > 0:
        for num in range(1, arg_results.vm_num + 1):
            for ip in ip_List:
                cfg_file.writelines("%s %s vm_%s_%s %s %s\n" % (arg_results.action, ip, arg_results.img_name,
                                                                num, arg_results.img_addr,
